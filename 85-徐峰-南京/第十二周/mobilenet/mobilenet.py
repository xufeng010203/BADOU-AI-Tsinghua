...
from keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, Conv2D
from keras.applications.imagenet_utils import decode_predictions
from keras import backend as K

input = Input(shape=[600, 600, 3])
x = Conv2D(32, (3, 3), strides=(1, 1), padding='valid')(input)
print(x)  # Tensor("conv2d_1/BiasAdd:0", shape=(?, 600, 600, 32), dtype=float32)
x = Conv2D(32, (3, 3), strides=(3, 3), padding='same')(input)
print(x)
#
...
def MobileNet(input_shape=[224,224, 3],
    ...
    x = _depthwise_conv_block(x, 128, depth_multiplier, strides=(2,2),
                              block_id=2)
...
def _depthwise_conv_block(ipnuts, pointwise_conv_filters,
                          depth_multiplier=1, strides=(1,1), block_id=1):
    x = DepthwiseConv2D((3,3), padding='same',
                        depth_multiplier=depth_multiplier,
                        strides=strides,
                        use_bias=False,
                        name='conv_dw_%d'%block_id)(ipnuts)
